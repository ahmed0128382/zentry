...
class TaskDetailsController
  ...
  void setPriority(TaskPriority newPriority) {
    ...
    state = AsyncData(
      current.copyWith(priorityDraft: newPriority),
    );
  }
  ...
  Future<void> toggleCompleted(bool value) async {
    ...
    try {
      ...
      // 3️⃣ Refresh both TaskDetails & main list
      final freshTask = await repo.getTaskById(current.task.id);
      if (freshTask != null) {
        state = AsyncData(
          state.value!.copyWith(
  ...
  Future<void> saveEdits() async {
    ...
    if (newTitle == current.task.title &&
        newDescription == current.task.description &&
        current.priorityDraft == current.task.priority) {
      return;
    }
    ...
    try {
      final updatedTask = current.task.copyWith(
        title: newTitle,
        description: newDescription,
        priority: current.priorityDraft, // ✅ persist priority
      );
